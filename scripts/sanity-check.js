...
const submissionDraftSource = fs.readFileSync(path.join(root, 'docs/wechat-submission-draft.md'), 'utf8')
const betaGuideSource = fs.readFileSync(path.join(root, 'docs/beta-test-guide.md'), 'utf8')
...
assertInvariant(profilePageSource.indexOf('小工具与记录') > -1 && profilePageSource.indexOf('actionIconPaths.tools') > -1 && profileLogicSource.indexOf('toolCount') > -1, 'Profile page should link to the unified tool hub with the toolbox icon')
...
assertInvariant(profileLogicSource.indexOf('submitPendingQuestion') > -1 && profilePageSource.indexOf('pending-question-input') > -1 && profileLogicSource.indexOf("addPendingQuestion(validation.text, 'profile')") > -1, 'Profile pending-question entry should let users submit new questions locally')
assertInvariant(profileLogicSource.indexOf('editable: true') === -1 && profilePageSource.indexOf('placeholder="例如：宝宝半夜总醒怎么办？"') > -1, 'Profile pending-question input should use a custom large textarea instead of wx.showModal editable')
assertInvariant(profileLogicSource.indexOf('只保存在这台手机') > -1, 'Pending-question pool should explain the current local-only backend boundary')
...
assertInvariant(submissionDraftSource.indexOf('无后台请求、无支付、无客服系统') > -1, 'Submission draft should keep MVP backend/payment/support boundaries for review notes')
assertInvariant(betaGuideSource.indexOf('当前版本还没有接后端') > -1 && betaGuideSource.indexOf('怎么反馈问题') > -1, 'Beta guide should explain local-only storage and feedback path')
...
assertInvariant(customTabBarStyleSource.indexOf('.tab-item.active .tab-icon-wrap') > -1 && customTabBarStyleSource.indexOf('background: transparent') > -1, 'Custom tabBar selected icon background should stay transparent to avoid crowding labels')
assertInvariant(customTabBarSource.indexOf('pressedIndex') === -1 && customTabBarLogicSource.indexOf('pressedIndex') === -1 && customTabBarStyleSource.indexOf('icon-press') === -1, 'Custom tabBar should avoid delayed jump animations during tab switching')
...
assertInvariant(profileLogicSource.indexOf('function normalizeDraftBaby') > -1 && profileLogicSource.indexOf("name: baby.name && baby.name !== '未设置' ? baby.name : ''") > -1, 'Profile baby edit form should not put unset display copy into editable inputs')
...
assertInvariant(recordsLogicSource.indexOf('showActionSheet') > -1 && recordsLogicSource.indexOf('tools.map((tool) => tool.title)') > -1 && recordsLogicSource.indexOf('url: tool.path') > -1, 'Records page new action should read tool choices from the tool registry')
...
assertInvariant(toolService.getToolHubTabs().some((item) => item.id === 'growth' && item.count === 1), 'Tool service should expose grouped tool hub tabs')
assertInvariant(toolService.getToolsByHubGroup('feeding').length === 1 && toolService.getToolsByHubGroup('feeding')[0].id === 'feeding_log', 'Tool hub groups should resolve to their registered tools')
